...
async function getFunnelMetrics(hoursAgo: number): Promise<FunnelMetrics> {
  ...
  return { ...metrics, utmSources: utmCounts } as any;
}

function buildReport(metrics: FunnelMetrics & { utmSources?: Record<string, number> }, period: string): string {
  ...
  // Health alerts
  const alerts: string[] = [];
  if (metrics.webapp_open_with_null > metrics.webapp_open * 0.3 && metrics.webapp_open > 0) {
  ...
  if (metrics.utmSources && Object.keys(metrics.utmSources).length > 0) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get metrics
    const metrics = await getFunnelMetrics(hours);
    const report = buildReport(metrics, `последние ${hours}ч`);
