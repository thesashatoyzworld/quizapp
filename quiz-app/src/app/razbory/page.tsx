...
// сервере (/api/cabinet/razbory), сюда контент приезжает уже после проверки —
// в клиентском бандле разборов нет.
...
function RazboryInner() {
  const [state, setState] = useState<'load' | 'guest' | 'locked' | 'ok'>('load');
  const [tier, setTier] = useState(0);
  const [items, setItems] = useState<Card[]>([]);
  const [tgId, setTgId] = useState<number | null>(null);
  // Открытый разбор: HTML конспекта рендерим в iframe, чтобы его стили не
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/cabinet/razbory${qs.toString() ? `?${qs}` : ''}`);
  ...
  async function openRazbor(card: Card) {
    ...
    try {
      ...
      if (preview) qs.set('preview', '1');
      const res = await fetch(`/api/cabinet/razbory?${qs}`);
      const data = await res.json();
  ...
  return (
    <main className="rz-wrap">
      ...
      <header className="rz-top">
        ...
        <div className="rz-brand">Разборы</div>
        <div className="rz-sub">Реальные созвоны учеников по шагам: что сработало, где посыпалось</div>
      </header>
      ...
      {state === 'guest' && (
        <div className="rz-card rz-login">
          ...
          <div className="rz-login-sub">
            Разборы привязаны к твоему Telegram. Войди, и раздел откроется, если у тебя тариф 2 или 3.
          </div>
          <TelegramLoginButton />
      ...
      {state === 'locked' && (
        <div className="rz-card rz-locked">
          ...
          <h1 className="rz-lock-h1">Разборы созвонов</h1>
          <p className="rz-lock-p">
            Здесь лежат записи разборов реальных продающих созвонов с конспектами по таймкодам.
      ...
      {state === 'ok' && items.length === 0 && (
        <div className="rz-card"><p className="rz-empty">Первый разбор появится здесь совсем скоро.</p></div>
      )}

      {state === 'ok' && items.map((c) => (
        <button className="rz-card rz-item" key={c.slug} onClick={() => openRazbor(c)}>
          ...
          <span className="rz-item-arr">{opening === c.slug ? 'открываю…' : 'Открыть разбор ↗'}</span>
        </button>
      ))}
...
export default function RazboryPage() {
  return <Suspense fallback={null}><RazboryInner /></Suspense>;
}
