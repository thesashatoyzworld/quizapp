// Статья урока «03-uroven-2» курса «Новый Уровень Контента».
// СГЕНЕРИРОВАНО: GSD-BRAND/scripts/kurs-to-ts.mjs 03 — руками не править,
// править исходник kurs/03-uroven-2.html и перегенерировать.

export const UROVEN_2_03 = `<!doctype html>
<html lang="ru">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="robots" content="noindex">
<title>Делал, но бросил · Новый уровень контента</title>
<style>
/* ============================================================
...
<header class="arthead"><div class="ahin">
  <div class="lvlbadge">Уровень 2</div>
  <h1>Делал, но бросил</h1>
  <p class="dek">По-хорошему это даже не отдельный уровень. Вы на первом, просто выключили игру.</p>
  <div class="taskline"><b>Задача:</b> Понять, что вас выбило, и вернуться с исправленными настройками.</div>
</div></header>
...
<main class="wrap">
<section>
<p>Если вы находитесь на этом уровне - поздравляю</p>
<p>У вас скорее всего уже есть накопленный опыт и первые набитые шишки</p>
<img src="/kurs/assets/ohrannik-7.png" alt="">
<p>Помните охранника с первого уровня? Он снял все замки и пустил вас внутрь</p>
<p>А теперь посмотрите, каким вы оттуда возвращаетесь: с фингалом, в порванной футболке, с разбитым телефоном</p>
<p>Получили по щам и вышли</p>
<p>Но взгляните на дверь - она открыта. Никто ее за вами не закрывал, и замки так и лежат на полу</p>
<p>Проблема данного уровня, что скорее всего вы делали контент через огромное сопротивление и с прицелом на результаты</p>
<p>В итоге - вы либо разочаровались из-за отсутствия результатов, либо выгорели в попытках получить эти результаты</p>
<p>Сразу проясню, что это за уровень</p>
<p>По-хорошему это даже не отдельный уровень</p>
<p>Вы находитесь на первом, просто вы выключили игру</p>
<p>Ничего не сломалось, прогресс никуда не делся</p>
<p>Вам нужно вернуться, разобраться, почему вы вышли, скорректировать действия и включить обратно</p>
<p>Именно поэтому здесь не будет новых инструментов</p>
<p>Здесь будет только разбор того, что с вами произошло, потому что без этого вы включите игру и выйдете из нее по той же самой причине</p>
</section>
<section>
<h2>Чем опасно застрять здесь</h2>
<p>Тем, что пауза превращается в вывод</p>
<p>Сначала это звучит как "я потом вернусь"</p>
<p>Потом как "видимо, это не мое"</p>
<p>А дальше человек сам себе доказывает, что он пробовал и у него не получилось</p>
<p>Или так: купили обучение, взяли формулы, поделали, джекпот не выбили - и бросили</p>
<p>Вспоминаем, что инстаграм это казино. Прийти и выбить джекпот с первого захода не получится</p>
<p>И каждый следующий заход становится тяжелее, потому что теперь у него есть "опыт неудачи"</p>
<p>Посмотрите еще раз на картинку с охранником. Если вы четыре раза выходите оттуда вот таким, вам четыре раза наваляли - ну конечно вы не захотите этим заниматься, потому что больно каждый раз</p>
<p>Поэтому я и говорю: важно выстроить это комфортным для себя способом</p>
<p>Хотя на самом деле никакой неудачи не было</p>
<p>Была остановка в предсказуемой точке, через которую проходят все</p>
</section>
<section>
<h2>Самое главное - это нормально</h2>
<div class="ix" data-ix="valley"></div>
<p>Нормально взять паузу</p>
<p>Нормально бросить что-то тяжелое и сложное</p>
<p>Нормально опустить руки</p>
<p>Мы все двигаемся волнообразно</p>
<p>Я хочу чтоб вы посмотрели следующую картинку и взяли ее на вооружение</p>
<img src="/kurs/assets/photo_2026-07-26_10-00-29.jpg" alt="">
<p>эта картинка описывает любое движение к определенной цели</p>
<p>Стадия 1. Неинформированный оптимизм.</p>
<p>Мы нашли какую-то идею, которая нас чрезвычайно вдохновляет и готовы с большой радостью окунутся в нее на 100%</p>
<p>Мы еще не знаем, что ждет нас впереди, но мы полны решимости и энтузиазма.</p>
<p>Стадия 2. Информированный пессимизм</p>
<p>В процессе пути мы обнаруживаем, что все не так просто.</p>
<p>Оказывается, у нас не хватает навыков или же мы недооценили сложность задачи.</p>
<p>В конце концов простые стечения обстоятельств могут мешать вам.</p>
<p>Вы уже понимаете, что все не так просто.</p>
<p>Стадия 3. Долина Отчаяния</p>
<p>Все валится из рук и ничего не получается.</p>
<p>Вы продолжаете двигаться на зубах, сила воли трещит по швам.</p>
<p>Вы выжжены и испытываете депрессивное состояние.</p>
<p>В этот момент мозг подкидывает вам НОВУЮ и КЛАССНУЮ идею (или же просто говорит "мы устали" и отказывается продолжать)</p>
<p>И вот здесь происходит поворотный момент - именно в этой долине отчаяния.</p>
<p>Но единственное, что их ждет через какое-то время - очередная долина отчаяния.</p>
<p>95% людей действуют именно таким способом.</p>
<p>Оставшиеся 5% продолжают и выбираются из этой долины - рано или поздно.</p>
<p>Стадия 4. Информированный оптимизм</p>
<p>Вы прошли сложное испытание и чувствуете как все начинает медленно работать.</p>
<p>Вы уже информированны, у вас получен опыт и вы снова смотрите в будущее с энтузиазмом.</p>
<p>Вы знаете, что вы очень близко к результату.</p>
<p>Стадия 5. Успех</p>
<p>Результат получен, вы дошли до него.</p>
<p>Ни навыки, ни деньги, ни знакомства не делают такую разницу, как способность проживать и пробираться через эту "долину отчаяния".</p>
<p>И это не про силу воли</p>
<p>Сила воли нам не нужна, мы про это говорили в предыдущих модулях. Это не про то, чтобы заставить себя и принудить. Это проходится по-другому</p>
<p>Теперь вы понимаете, почему я предлагаю не оценивать цифры и результаты сразу?</p>
<p>Потому что как только мы с вами преодолеем первую "долину отчания" - у нас сформируется нейронный мост в голове, который будет звучать так:</p>
<p>"Это возможно"</p>
<p>И мост этот не про количество подписчиков. Подписчики здесь вообще не имеют значения. Мост про то, что я способен проходить эту долину</p>
</section>
<section>
<h2>Жизнь проверяет твердость намерения</h2>
<p>Вы не представляете, какое количество людей я за последние три года повстречал, которые ломались именно здесь</p>
<p>У меня есть друг, который до сих пор не разобрался, как эту долину проходить</p>
<p>Это уровень в игре, он важен и нужен</p>
<p>Жизнь проверяет твердость вашего намерения: насколько вы действительно хотите реализовать эту цель, насколько вы к ней готовы</p>
<p>И если вы каждый раз сворачиваете, жизнь говорит: ну ладно, значит скорее всего ты этого не хочешь, ты просто делаешь вид</p>
<p>Наша задача - создать максимально комфортные условия для прохождения этой долины</p>
<p>Упростить для себя задачу настолько, чтобы она не вызывала трудностей</p>
<p>Почему?</p>
<p>Показываю следующую картинку</p>
<img src="/kurs/assets/potok.png" alt="">
<p>Если задача сложная, а навыка не хватает - вы находитесь в зоне тревоги и стресса</p>
<p>Долго там находится невозможно, поэтому в какой-то момент человек бросает (психика не выдерживает)</p>
<p>Расход энергии в этой зоне огромный, и энергия очень быстро заканчивается. Как на Эвересте заканчивается кислород</p>
<p>Если задача простая, а навыка прокачан - вы находитесь в зоне скуки</p>
<p>Долго там находится невозможно, вы просто перестаете это делать и ищете что-то новое</p>
<p>И только когда сложность задачи соотносятся с вашим настоящим уровнем - вы входите в состояние потока и способы действовать</p>
<p>Поэтому я и предлагаю держать планку сложности всегда на минимальном уровне, потому что знаю, как люди в 99% переоценивают свои возможности</p>
<p>Тех, кому было скучно, я почти не встречал</p>
<p>Встречаю я других: эпоха достигаторства, человек дает себе челлендж, издевается над собой, закидывает планку - и разбивается об нее</p>
<p>Это как пытаться перепрыгнуть сразу несколько ступенек</p>
</section>
<section>
<h2>Почему вы вышли из игры</h2>
<div class="ix" data-ix="causes"></div>
<p>Прежде чем включать обратно, надо понять, что именно вас выбило</p>
<p>Вариантов немного, и они почти всегда из этого списка:</p>
<ul><li><strong>вы ждали результатов</strong> и не увидели их в тот срок, который сами себе назначили</li><li><strong>вы задрали планку сложности</strong> - снимали слишком долго, монтировали слишком тщательно, требовали от себя слишком много</li><li><strong>вы делали не в той форме</strong> - писали, хотя вам комфортнее говорить, или снимали, хотя вам проще писать</li><li><strong>вы говорили не свое</strong> - брали чужие темы и чужие смыслы, потому что они "должны работать"</li><li><strong>у вас изменился контекст</strong> - переезд, работа, ребенок, здоровье. Тогда это вообще не про контент</li></ul>
<p>Разница принципиальная</p>
<p>Если вас выбило ожиданиями или сложностью - вы возвращаетесь на первый уровень и корректируете действия</p>
<p>Если вас выбило формой или чужими смыслами - вам нужен третий уровень, там мы это разбираем подробно</p>
<p>Если изменился контекст - вы ничего не бросали, вы занимались жизнью, и это правильно</p>
</section>
<section>
<span class="slabel">коротко</span><h2>Саммари</h2>
<p>Второй уровень - это не про то, что у вас не получилось</p>
<p>Это про то, что вы дошли до долины отчаяния и вышли из игры именно там, где выходит большинство</p>
<p>Задача здесь одна: разобраться, что вас выбило, и вернуться с исправленными настройками</p>
<p>Не с новой стратегией, не с новым обучением и не с понедельника</p>
<p>С теми же действиями первого уровня, только с пониженной планкой</p>
</section>
<section>
<span class="slabel">делаем руками</span><h2>Задача</h2><div class="taskbox"><div class="tt">задача уровня</div>
<ul><li>1. Выпишите честно, почему вы бросили в прошлый раз. Не "не было времени", а что конкретно произошло: сколько единиц сделали, чего ждали, что получили, в какой момент перестали</li></ul>
<p>Если самому копать тяжело - под это есть промпт: он задает вопросы по одному, не принимает "не было времени" и на выходе называет причину, показывает, куда вам идти, и собирает ваши новые условия на возвращение → <i>Промпт — почему я бросил</i></p>
<ul><li>2. Посмотрите на свой список и найдите, что из этого вы можете убрать или упростить прямо сейчас. Меньше монтажа, короче формат, другая площадка, другая форма</li></ul>
<ul><li>3. Вернитесь на первый уровень и сделайте одну единицу по новым, упрощенным правилам. Одну, а не десять</li></ul>
<p>Не надо новой стратегии и не надо с понедельника. Прямо сейчас поставьте урок на паузу, включите камеру, скажите в нее что-нибудь и выложите. Те же самые действия первого уровня, только с пониженной планкой</p>
<ul><li>4. Дайте себе две недели без оценки результатов, а лучше месяц. Просто снимайте, кайфуйте, говорите что хотите, несите любую чушь. Единственный критерий - вы продолжаете</li></ul>
</div></section>
<section>
...
<p>Вы снова выкладываете, и мысль "а вдруг опять не пойдет" больше не останавливает вас перед публикацией</p>
<p>Вы знаете, на чем вас выбило в прошлый раз, и видите, что в этот раз условия другие</p>
<p>Закрепитесь на уровне "делания" контента</p>
<p>И после этого переходите к следующему.</p>
</div></section>
<section>
<h2>До цели доходит тот, кто продолжает</h2>
<p>С вами все нормально</p>
<p>Пауза это нормально. День и ночь, прилив и отлив, работа и отдых - с контентом ровно так же</p>
<p>Поделали, что-то пошло не так, собрали информацию, остановились</p>
<p>Главное - взять эту информацию и обработать ее. Потом вернуться и включить обратно</p>
<p>Может быть, вы остановитесь еще раз, чтобы понять про себя что-то новое. Это тоже нормально</p>
<p>До цели доходит тот, кто продолжает движение</p>
<p>И никогда это не выглядит так, как вам показывают в инстаграме. Там показывают либо "я взлетел", либо "я ничего не делал, а потом сразу в космос"</p>
<p>Чаще всего дорога выглядит по-другому: сходить вниз, потоптаться там, потом улететь вверх, потом снова вниз</p>
<p>Но на дистанции тренд всегда будет положительным, если вы продолжаете</p>
</section>
