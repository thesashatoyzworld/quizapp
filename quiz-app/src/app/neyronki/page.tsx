...
// Раздел «Нейронки» - записи экрана и конспекты: как собрать своими руками то,
// за что обычно платят подрядчику. Доступ: активный «Новый уровень контента»
// тарифа 2 или 3. Проверка живёт на сервере (/api/cabinet/neyronki), сюда
// контент приезжает уже после проверки, в клиентском бандле его нет.
...
function NeyronkiInner() {
  ...
  // Открытый материал: HTML конспекта рендерим в iframe, чтобы его стили не
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/cabinet/neyronki${qs.toString() ? `?${qs}` : ''}`);
  ...
  async function openItem(card: Card) {
    ...
    try {
      ...
      if (preview) qs.set('preview', '1');
      const res = await fetch(`/api/cabinet/neyronki?${qs}`);
      const data = await res.json();
  ...
  return (
    <main className="rz-wrap">
      ...
      <header className="rz-top">
        ...
        <div className="rz-brand">Нейронки</div>
        <div className="rz-sub">Собираем руками то, за что обычно платят подрядчику: сайт, бот, автоматизация</div>
      </header>
      ...
      {state === 'guest' && (
        <div className="rz-card rz-login">
          ...
          <div className="rz-login-sub">
            Раздел привязан к твоему Telegram. Войди, и он откроется, если у тебя тариф 2 или 3.
          </div>
          <TelegramLoginButton />
      ...
      {state === 'locked' && (
        <div className="rz-card rz-locked">
          ...
          <h1 className="rz-lock-h1">Нейронки</h1>
          <p className="rz-lock-p">
            Здесь лежат записи экрана и конспекты по таймкодам: как через нейронку собрать
            себе сайт, бота и всё остальное, за что обычно платят подрядчику.
      ...
      {state === 'ok' && items.length === 0 && (
        <div className="rz-card"><p className="rz-empty">Первый материал появится здесь совсем скоро.</p></div>
      )}

      {state === 'ok' && items.map((c) => (
        <button className="rz-card rz-item" key={c.slug} onClick={() => openItem(c)}>
          ...
          <span className="rz-item-arr">{opening === c.slug ? 'открываю…' : 'Открыть ↗'}</span>
        </button>
      ))}
...
export default function NeyronkiPage() {
  return <Suspense fallback={null}><NeyronkiInner /></Suspense>;
}
