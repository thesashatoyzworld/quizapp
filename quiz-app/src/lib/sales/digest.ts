 ...
 *
 * Вызывается на каждое сообщение в личке, и на входящее, и на наш ответ, но
...
export async function pushDigest(): Promise<void> {
  ...
  for (const chatId of helpers()) {
    ...
    // Пустую очередь новым сообщением не шлём: «Все отвечены» само по себе
    // не новость, а сводка зовётся теперь и на наших ответах — иначе каждые
    // три четверти часа в чат падало бы сообщение ни о чём.
