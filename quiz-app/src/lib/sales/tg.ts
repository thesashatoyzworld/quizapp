...
export async function handleBusinessMessage(msg: TgBusinessMessage): Promise<void> {
  ...
  // Ответили сами — сводку надо поправить тут же. Раньше она обновлялась
  // только на входящих, и человек, которому Саша только что написал, висел
  // в «ждут ответа» до чьей-нибудь следующей реплики: 11.09 между правками
  // сводки прошло больше четырёх часов и тридцати шести наших ответов,
  // то есть в телеграме висел список, наполовину состоящий из отвеченных.
  if (side !== 'client') {
    await pushDigest();
    return;
  }
